...
const onlyTitles = recipes.map((food) => food.title); // ['Crepes', ...]
console.log(onlyTitles);

const titlesWithDuration = recipes.map(
  (food) => food.title + " (" + food.duration + " min)"
); // ['Crepes (60min)', ...]
console.log(titlesWithDuration);

const timePerServing = recipes.map((food) => food.duration / food.servings); // [20, 10, 432, 12]
...
const allTitlesInOneString = recipes.map((food) => food.title).join(", "); // 'Crepes, Scrambled Eggs, ...'
